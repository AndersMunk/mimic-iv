CREATE OR REPLACE TABLE `aki-recovery-prediction.MimicIV.MIMIC_kdigo_stages` AS
with cr_stg AS
(
  SELECT
    cr.stay_id
  ...
  FROM `aki-recovery-prediction.MimicIV.MIMIC_kdigo_creat` cr
)
-- stages for UO / creat
, uo_stg as
(
  select
    ...
    , CASE
        ...
        WHEN uo.uo_tm_24hr >= 23 AND uo.uo_rt_24hr < 0.3 THEN 3
        WHEN uo.uo_tm_12hr >= 11 AND uo.uo_rt_12hr = 0 THEN 3
        WHEN uo.uo_tm_12hr >= 11 AND uo.uo_rt_12hr < 0.5 THEN 2
        WHEN uo.uo_tm_6hr >= 5 AND uo.uo_rt_6hr  < 0.5 THEN 1
    ELSE 0 END AS aki_stage_uo
  from `aki-recovery-prediction.MimicIV.MIMICIV_kdigo_uo` uo
